...
int SocketEpoll::handle_event(epoll_event &e) { //对事件进行分类处理
...
int SocketEpoll::start_epoll() {
    ...
    // std::cout << "Go To start epoll loop" << std::endl;
...
int SocketEpoll::start_epoll_loop() {
    ...
    while (_status != S_STOP)
    {
        ...
        //相当于select的调用 用于得到时间集合 返回需要执行时间的数量 
        if(epoll_events_count == -1) //返回-1表示出现错误 需要用errno判断错误类型
        {
            if(errno == EINTR)
            {
        ...
        for(int i = 0; i < epoll_events_count; i++) //epoll处理事件
