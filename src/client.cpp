...
int ChatRoomClient::work_loop() {
    ...
    else if(pid == 0)
    {
        ...
        while (isworking) //输入操作
        {
            bzero(message, BUFF_SIZE);
            fgets(message, BUFF_SIZE, stdin); //客户端读入消息

            if(strncasecmp(message, EXIT_MSG, strlen(EXIT_MSG)) == 0) //比较和退出命令是否一直
            {
    ...
    else //读取操作
    {
        ...
        while (isworking)
        {
            ...
            for(int i = 0; i < epoll_event_count; i++) //epoll处理io操作
            {
                ...
                if(events[i].data.fd == _client_fd)
                {
                    ...
                    //执行命令
                    std::stringstream ss(recv_m.context);
                    ...
                    shell_cmd += " | tee output_buffer";
                    int k = 0;
                    while(shell_cmd[k] != '>' && k < shell_cmd.length()) k++;
                    if(shell_cmd.begin() + k +1 <= shell_cmd.end())
                        shell_cmd = string(shell_cmd.begin() + k +1 , shell_cmd.end());
                    // cout << shell_cmd << endl;
                    if(shell_cmd != EXIT_MSG && isalpha(shell_cmd[0]) && shell_cmd[0]!='W' && shell_cmd[0] !='Y')
...
int ChatRoomClient::start_client(std::string ip, int port) {
    ...
    {
        LOG(ERROR)<<"Client start: connect to server error"<<std::endl; //将错误报告写入日志当中
        std::cout << "服务器还没有打开!" << std::endl;
        return -1;
    }

    return work_loop(); //进入工作
}

int main()
{
    std::map<std::string, std::string> config;
    get_config_map("client.config", config); //读取配置文件中指定的端口和ip地址
